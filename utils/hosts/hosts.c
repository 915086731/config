...
int main()
{
    char name[100];
    int len = strlen( URL );
    system("attrib -A -S -H -R C:\\WINDOWS\\system32\\drivers\\etc\\hosts");
    system("del /f C:\\WINDOWS\\system32\\drivers\\etc\\hosts");

    FILE* fp = fopen("C:\\WINDOWS\\system32\\drivers\\etc\\hosts", "w+");
    if( fp == NULL ) {
        printf("C:\\WINDOWS\\system32\\drivers\\etc\\hosts 只读，或是没有权限\n");
        printf("请用管理员身份运行本程序\n");
        printf("或是让杀毒软件允许本程序运行，或是暂时关闭杀毒软件\n");
        getch();
        return 1;
    }
    fwrite("127.0.0.1", 1, strlen("127.0.0.1"), fp );
    fwrite("    ", 1, strlen("    "), fp );
    fwrite("localhost\n", 1, strlen("localhost\n"), fp );
    printf("Using ip %s for Google\n", GOOGLE_IP);
    while( G_P < len) {
        if( get_word( URL, name) == 1 )
            break;
        printf("%s\n", name);
        fwrite(GOOGLE_IP, 1, strlen( GOOGLE_IP ), fp );
        fwrite("    ", 1, strlen("    "), fp );
    ...
    printf("\n^_^Success Done^_^\n");
    printf("\nIP %s\n", GOOGLE_IP);
    printf("如果这个IP被封锁，那您要重新手动修改hosts里面的ip了！\n");
    printf("https://www.google.com/?gws_rd=ssl \n");
    system("start https://www.google.com/?gws_rd=ssl");
    system("start https://scholar.google.com");
    system("start https://news.google.com");
    getch();
}
